...
def search_master_data(
    ...
    **CRITICAL VALIDATION RULES**:
    ...
    Examples:
       ...
       2. FALLBACK ONLY - Full-text search when specific field search fails:
          search_master_data(
              search_type="record",
              query={"expressions": [{"property": "*", "condition": "contains", "value": "Smith"}]}
          )
       ...
       4. Search with filters - Find person records with last name "Smith":
           search_master_data(
               search_type="record",
               query={"expressions": [{"property": "legal_name.last_name", "condition": "equal", "value": "Smith"}]},
       ...
       5. Browse all entities - Get sample data (use sparingly with small limit):
          search_master_data(
              search_type="entity",
    ...
    """
    # Validate via Pydantic model before calling service
    validated = SearchMasterDataRequest(
    ...
    result = service.search_master_data(
        ...
        include_total_count=validated.include_total_count,
        crn=validated.crn,
        include_attributes=validated.include_attributes,
        exclude_attributes=validated.exclude_attributes,
    )
